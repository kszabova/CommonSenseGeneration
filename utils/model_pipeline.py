class ModelPipeline:
    ...
    def model_step(self, model, prev_outputs, input_fn, output_fn, device):
        return output_fn(
            model.generate(input_fn(prev_outputs).to(device)), prev_outputs
        )
    ...
    def run_pipeline(self, device):
        for data in self.data:
            ...
            for model, input_fn, output_fn in zip(
                ...
                prev_outputs = self.model_step(
                    model, prev_outputs, input_fn, output_fn, device
                )
            yield prev_outputs
